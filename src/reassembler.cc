...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  if ( data.empty() ) {
    ...
    debug( "Data empty after trimming, nothing to insert." );
    return;
  }
  // 处理重叠和存储
  // 使用的就是reassembler中定义的 segments_ 这个 map 既缓存区，就是图中 unassembled segments 部分
  // 1. 先处理与已存储数据块的重叠, 向前合并
  ...
  while ( !segments_.empty() && segments_.begin()->first == next_index_ ) {
    ...
    debug( "Writing segment to output at index {}: \"{}\"", segments_.begin()->first, seg );
  ...
  if ( eof_received_ && unassembled_bytes_ == 0 && next_index_ >= eof_index_ ) {
    debug( "All data written and EOF received, closing output." );
    output_.writer().close();
  }
  debug( "--- Insert end ---" );
}
